...
x_new = np.array([60, 90])
y_new = np.array([0.6, 0.045])

plt.plot(x_new, y_new, 'r--', label='(v-0.041)/1.66 =-(lat-90)/90')  # 'b-' means a solid blue line
# 第一个图例（slit 7, slit 5, slit 3）
plt.plot(90, 0.00221, 'o', color='white', markeredgecolor='black', markersize=6, markeredgewidth=1.5,label='')

# Plotting the slits data
plt.scatter(latitudes_slit7, speeds_slit7, color='red', label='slit 7')
plt.scatter(latitudes_slit5, speeds_slit5, color='blue', label='slit 5')
plt.scatter(latitudes_slit3, speeds_slit3, color='lightgreen', label='slit 3')
plt.scatter(latitudes_slit5N, speeds_slit5N, color='black', label='slit 5N')
# 第二个图例（slit 5N）
...
legend1 = plt.legend(handles1[0:1], labels1[0:1], loc='upper right', fontsize=14, bbox_to_anchor=(1, 0.95),edgecolor="none",prop={'weight': 'bold','size':13})
ax.text(72.5, 0.83, "rms dev = 0.0221",fontsize=13,fontweight='bold')
